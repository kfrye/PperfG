...
for f in dir_list:
  slurm = [d for d in listdir_fullpath(f) if 'slurm' in d][0]
  slurm = os.path.basename(slurm)
  regex = 'slurm-([0-9]+)'
  match = re.search(regex, slurm)
  slurm = match.group(1)
  inner_dirs = [d for d in listdir_fullpath(f) if 'darshan' in d and os.path.isdir(d)]
  ...
  for input_dir in inner_dirs:
    ...
    month = date.month
    day = date.day
    regex = 'darshan_mpirun_bt.([A-Z]).([0-9]+).'
    match = re.search(regex, f)
    size = match.group(1)
    ranks = match.group(2)
    darshan_directory = '/usr/projects/darshan/logs/' + str(year) + '/' + \
                        str(month) + '/' + str(day)
    darshan_list = [d for d in listdir_fullpath(darshan_directory) if 'bt.' + size + '.' + ranks in d and slurm in d]
    darshan_list = sorted(darshan_list,key=os.path.getctime)
    
    if len(darshan_list) != len(file_list):
      print darshan_list
      print file_list
